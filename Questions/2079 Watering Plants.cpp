...
int wateringPlants(vector<int>& plants, int capacity) {
        int idx=0;//store the index we have to fill next
        int can=capacity;
        int steps=0;
        while(idx<plants.size()){
            if(plants[idx]<=can){
                // if we are able to fill the plant at idx with remaining water in jug
                steps++;
                can-=plants[idx];
                idx++;
                cout<<"\n 1. filled water at "<<idx<<" no of steps are "<<steps<<" can have "<<can<<" amounts"<<endl;
            }
            else{
                steps+=2*(idx);
                can=capacity;
                cout<<"\n 2. refilled water at "<<idx<<" no of steps are "<<steps<<" can have "<<can<<" amounts"<<endl;
...
int main(){
 ...
 plants.push_back(3);
 plants.push_back(3);
 cout<<endl<<wateringPlants(plants,5);//ans=14
  return 1;
}
